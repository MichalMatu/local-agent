...
This file records the release/runtime invariants for `MichalMatu/local-agent`. The source release is `v4.18.18` and the current production release is `v4.18.18`. Runtime and browser behavior were validated live on `326e31e76700304ff18c744e24e71cce876484ac`: production naturally self-updated to Local Agent 4.18.18 without a manual restart, Chat Bridge 0.5.7 upgraded the already-open unreloaded ChatGPT tab to content protocol v6 after extension Reload, and `[LAB:HELP]` returned `[LA_BRIDGE_FEEDBACK]` through automatic Send with no operator submission. `rollback/v4.18.18-production-validated` preserves that exact runtime/browser-validation point. The previous tagged frozen release remains `v4.18.15` at `338dd59e16e2c52c8148625e4f7bd2551aab92e8` until the final 4.18.18 documentation-freeze commit is tagged; `rollback/v4.18.15-production-validated` preserves its earlier exact runtime-validation point. The older `v4.18.13` / `a32e54858c3bcb9687334b3232b71ae6ff130208` baseline remains available as the pre-BUG-002 historical rollback point.
