...
def _remote_ref(self_repo: Path) -> str:
    ...
    if result["exit_code"] != 0:
        raise RuntimeError(str(result.get("output", "")).strip() or "operator control ref probe failed")
...
def poll_remote_operator(
    ...
    *,
    ...
    Remote `enabled` never clears the local marker. Re-enabling always requires an
    explicit local operator action after the remote desired state was cleared.
    """
