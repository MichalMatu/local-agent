...
def activate_next_lineage_revision(
    ...
    are projected from authoritative effective state. The operation is restart-safe:
    activation records are create-only/idempotent and effective-state application is
    pinned to the exact revision the caller observed before mutation.
    """
    workflow_store.load_manifest(workflow_id)
    revision_store = WorkflowRevisionStore(workflow_store)
    activation_store = WorkflowRevisionActivationStore(workflow_store, revision_store)
    ...
    if os.path.lexists(effective_path):
        ...
        effective_state = effective_store.activate_next_revision(
            workflow_id,
