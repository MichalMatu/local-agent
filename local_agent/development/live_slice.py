...
explicitly armed capability lease for one bounded live browser spawn. It never
starts a Local Agent executor, installs Native Messaging, or uses the operator's
...
def build_live_slice_plan(
    ...
    """Build a read-only one-child live plan from one admitted ChildRequest."""
    validate_dev_lab_layout(layout)
...
def prepare_live_slice(
    ...
    """Persist one exact live plan without granting permission to launch it."""
    plan = build_live_slice_plan(layout, request)
    ...
    if paths.plan.exists() or paths.plan.is_symlink():
        ...
        if existing != document:
            raise RuntimeError("existing live slice plan differs; disarm/clear it explicitly first")
        return existing
...
def arm_live_slice(
    ...
    *,
    ...
    """Create a short-lived one-shot launch lease for one exact prepared plan."""
...
def consume_live_slice_arm(
    ...
    *,
    ...
    """Consume the arm before the first external browser effect."""
    document = load_prepared_live_slice(layout)
...
def live_slice_status(
    ...
    *,
    ...
    return {
        ...
        "armed": arm is not None and not expired,
        "expired": expired,
        "plan_digest": document.get("plan_digest") if document else None,
        "launch_nonce": arm.get("launch_nonce") if arm and not expired else None,
...
def _load_request(path: Path) -> dict[str, Any]:
    payload = _load_object(path, field="child request")
    contract.validate_child_request(payload)
    return payload
...
def main(argv: Iterable[str] | None = None) -> int:
    try:
        ...
        if args.command in {"plan", "prepare"}:
            if not args.request:
                raise ValueError("--request is required for plan/prepare")
            request = _load_request(Path(args.request))
            if args.command == "plan":
                payload = _plan_document(build_live_slice_plan(layout, request))
            else:
                payload = prepare_live_slice(layout, request)
